...
.\n` +
   '╭────────────────────────────╮\n' +
  '│ 🔐 كيف تود تسجيل الدخول يا دون؟ │\n' +
  '╰────────────────────────────╯\n' +
  '\n' +
  '📷 اكتب *qr* لمسح رمز الاستجابة السريعة.\n' +
  '🔢 أو اكتب *code* لإدخال رمز مكوّن من 8 أرقام.\n' +
  '\n' +
  '> '
  )
);
  ...
  if (loginMethod === 'code') {
    let phoneNumber = await question(chalk.blue('اكتب رقمك الان مثال: 201286015658:\n'));
    phoneNumber = phoneNumber.replace(/\D/g, ''); // 𝑷𝒐𝒘𝒆𝒓
...
async function connectionUpdate(update) {
  ...
  const reason = new Boom(lastDisconnect?.error)?.output?.statusCode;
  if (reason === 405) {
    if (existsSync('./Taibsessions/creds.json')) unlinkSync('./Taibsessions/creds.json');
    console.log(
